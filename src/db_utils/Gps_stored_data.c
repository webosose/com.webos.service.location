...
int gps_plugin_get_stored_position(Position *position, Accuracy *accuracy)
{
    ...
    xmlChar *result = NULL;
    int ret;
    if (handle == NULL) return ERROR_NOT_AVAILABLE;
    else {

        if (isFileExists() == 0) initializeLocationDB();
        handle->fileName = LOCATION_DB_PREF_PATH;
...
int gps_plugin_get_stored_velocity(guint64 *timestamp, gdouble *speed,
                                   gdouble *direction, gdouble * climb,
                                   gdouble *hor_accuracy, gdouble *ver_accuracy)
{
    ...
    if (handle == NULL) return ERROR_NOT_AVAILABLE;
        ...
        ret = get(handle, "timestamp", &result);
        if (ret == KEY_NOT_FOUND) return ERROR_NOT_AVAILABLE;
        else *timestamp = atoll(result);

        ret = get(handle, "speed", &result);

        if (ret == KEY_NOT_FOUND) return ERROR_NOT_AVAILABLE;
        else *speed = atof(result);

        ret = get(handle, "direction", &result);
        if (ret == KEY_NOT_FOUND) return ERROR_NOT_AVAILABLE;
        else *direction = atof(result);

        ret = get(handle, "climb", &result);
        if (ret == KEY_NOT_FOUND) return ERROR_NOT_AVAILABLE;
        else *climb = atof(result);

        ret = get(handle, "hor_accuracy", &result);
        if (ret == KEY_NOT_FOUND) return ERROR_NOT_AVAILABLE;
        else *hor_accuracy = atof(result);
        ret = get(handle, "ver_accuracy", &result);
        if (ret == KEY_NOT_FOUND) return ERROR_NOT_AVAILABLE;
        else *ver_accuracy = atof(result);
...
int cell_get_stored_position(Position *position, Accuracy *accuracy)
{
    ...
    if (handle == NULL) return ERROR_NOT_AVAILABLE;
        ...
        handle->fileName = LOCATION_DB_PREF_PATH;
        ret = get(handle, "cell_timestamp", &result);
        if (ret == KEY_NOT_FOUND) return ERROR_NOT_AVAILABLE;
        else position->timestamp = atoll(result);
        ret = get(handle, "cell_latitude", &result);
        if (ret == KEY_NOT_FOUND) return ERROR_NOT_AVAILABLE;
        else position->latitude = atof(result);

        ret = get(handle, "cell_longitude", &result);
        if (ret == KEY_NOT_FOUND) return ERROR_NOT_AVAILABLE;
        else position->longitude = atof(result);

        ret = get(handle, "cell_hor_accuracy", &result);
        if (ret == KEY_NOT_FOUND) return ERROR_NOT_AVAILABLE;
        else accuracy->horizAccuracy = atof(result);

        ret = get(handle, "cell_ver_accuracy", &result);
        if (ret == KEY_NOT_FOUND) return ERROR_NOT_AVAILABLE;
        else accuracy->vertAccuracy = atof(result);
...
int wifi_plugin_get_stored_position(Position *position, Accuracy *accuracy)
{
    ...
    if (handle == NULL) return ERROR_NOT_AVAILABLE;
        ...
        handle->fileName = LOCATION_DB_PREF_PATH_CELL;
                              ...
                              gdouble ver_accuracy)
{
    ...
    handle = (DBHandle*) malloc(sizeof(DBHandle));
    if (handle != NULL) {
        createPreference(LOCATION_DB_PREF_PATH_CELL, handle, "Location\n",
                         FALSE);
        char input[50];
        ...
        sprintf(input, "%lf", latitude);
        put(handle, "cell_latitude", input);

        sprintf(input, "%lf", longitude);
        put(handle, "cell_longitude", input);

        sprintf(input, "%lf", hor_accuracy);
        put(handle, "cell_hor_accuracy", input);
        sprintf(input, "%lf", ver_accuracy);
        put(handle, "cell_ver_accuracy", input);
