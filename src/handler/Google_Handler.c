...
G_DEFINE_TYPE_WITH_CODE(GoogleHandler, google_handler, G_TYPE_OBJECT,
        G_IMPLEMENT_INTERFACE (HANDLER_TYPE_INTERFACE, google_handler_interface_init));
 ...
 * @return    int
 */
void google_handler_tracking_cb(gboolean enable_cb ,Position *position,  Accuracy *accuracy, int error,
 ...
 * @return    int
 */
void google_handler_position_cb(gboolean enable_cb ,Position *position, Accuracy *accuracy,
        int error, gpointer privateIns)
{
...
parse_send_response (char *body,gpointer self, GoogleHandlerMethod method)
{
    ...
    if(method == GOOGLE_GET_POSITION)
        google_handler_position_cb(TRUE ,ret_pos, ac,
                ERROR_NONE, self);
    else
        google_handler_tracking_cb(TRUE ,ret_pos, ac, 0,
                self, HANDLER_GOOGLE_WIFI_CELL);
 ...
 * @return    int
 */
static void google_handler_start_tracking(Handler *self, gboolean enable ,StartTrackingCallBack track_cb ,gpointer agent_userdata, gpointer handlerobj)
{
    LS_LOG_DEBUG("[DEBUG]google handler start Tracking called agent_userdata=%s",(char*)agent_userdata);
    int result = ERROR_NONE;
    GoogleHandlerPrivate* priv = GET_PRIVATE(self);
    if(priv == NULL)
        track_cb(TRUE ,NULL,NULL, ERROR_NOT_AVAILABLE, NULL , 4);
 ...
 * @return    int
 */
static int google_handler_geo_code(Handler *self ,Address* address ,GeoCodeCallback geo_cb  )
{
    int ret = ERROR_NOT_APPLICABLE_TO_THIS_HANDLER;
    return ret;
}
 ...
 * @return    int
 */
static int google_handler_reverse_geo_code(Handler *self ,Position* pos,RevGeocodeCallback rev_geo_cb )
{
    int ret = ERROR_NOT_APPLICABLE_TO_THIS_HANDLER;
    return ret;
}
