...
class LocationService
{
    public:
        class TimerData
        {
            public:
                ...
                LSHandle* GetHandle() const {return m_sh;};
                LSMessage* GetMessage() const {return m_message;};
                unsigned char GetHandlerType() const {return m_handlerType;};
            private:
                LSMessage *m_message;
                LSHandle *m_sh;
        ...
        class Request {
            public:
                ...
                LSHandle* GetHandle() const {return m_sh;};
                LSMessage* GetMessage() const {return m_msg;};
                guint GetTimerID() const {return m_timerID;};
                TimerData* GetTimerData() {return m_timerdata;};
                unsigned char GetReqHandlerType() const {return m_handlerType;};
        ...
        bool init();
        static LocationService *getInstance(GMainLoop *);
        ...
        {
            getInstance(LocationService::mainLoop)->startTracking_reply(pos , accuracy, error, type);
        }
        static void get_nmea_cb(gboolean enable_cb,int timestamp, char *data, int length, gpointer privateIns)
        {
            ...
            getInstance(LocationService::mainLoop)->get_nmea_reply(timestamp,data,length);
        }

        static void getCurrentPosition_cb(Position* pos, Accuracy* accuracy, int error ,gpointer privateIns, int handlerType)
        {
            getInstance(LocationService::mainLoop)->getCurrentPosition_reply(pos,accuracy,error,handlerType);
        }

        static void getGpsSatelliteData_cb(gboolean enable_cb,Satellite *satellite,
                gpointer privateIns)
        {
            getInstance(LocationService::mainLoop)->getGpsSatelliteData_reply(satellite);
        }

        static int CreateSharedPref();
        static int getHandlerStatus(const char *);
        static void setGmainloop(GMainLoop *);
        static bool GetWifiApsList_cb(LSHandle *sh, LSMessage *reply, void *ctx)
        {
            return getInstance(LocationService::mainLoop)->_GetWifiApsList_cb(sh,reply,ctx);
        }
        static gboolean TimerCallback(void* data)
        {
            return getInstance(LocationService::mainLoop)->_TimerCallback(data);
        }
        ...
        {
            wifistate = true;//state;
        }
        void updateConntionManagerState(bool state)
        {
            isInternetConnectionAvailable = true;//state; for TESTING
    ...
    private:
        ...
        LocationService(GMainLoop *mainLoop);
        ...
        bool readLocationfromCache(LSHandle *sh,LSMessage *message,json_object *serviceObject,int maxAge,HandlerStatus handlerstatus);
        void handler_Selection(LSHandle *sh, LSMessage* message,void* data,int *,int *,HandlerStatus *);
        ...
        void stopSubcription(Handler *hdl,const char* key);
