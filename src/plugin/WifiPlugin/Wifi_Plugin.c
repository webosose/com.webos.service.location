...
static int get_position(gpointer handle, PositionCallback positionCB)
{
    ...
    g_signal_connect(G_OBJECT(GEOCLUE_PROVIDER(geoclueWifi->geoclue_pos)), "position-changed", G_CALLBACK(position_cb), geoclueWifi);
    return ERROR_NONE;
}
...
static void status_cb(GeoclueProvider *provider, gint status, gpointer userdata)
{
    ...
    case GEOCLUE_STATUS_UNAVAILABLE:
    {
        ...
        g_signal_handlers_disconnect_by_func(G_OBJECT(GEOCLUE_PROVIDER(plugin_data->geoclue_pos)), G_CALLBACK(status_cb), plugin_data);
    }
        break;
    default:
        break;
...
static int start_tracking(gpointer plugin_data, gboolean enableTracking, StartTrackingCallBack handler_tracking_cb)
{
    ...
    if (enableTracking) {
        ...
        g_signal_connect(G_OBJECT(GEOCLUE_PROVIDER(geoclueWifi->geoclue_pos)), "position-changed", G_CALLBACK(tracking_cb), plugin_data);
        ...
        g_signal_handlers_disconnect_by_func(G_OBJECT(GEOCLUE_PROVIDER(geoclueWifi->geoclue_pos)), G_CALLBACK(tracking_cb), plugin_data);
        g_signal_handlers_disconnect_by_func(G_OBJECT(GEOCLUE_PROVIDER(geoclueWifi->geoclue_pos)), G_CALLBACK(status_cb), geoclueWifi);
...
if(wifid == NULL)
return NULL;
wifid->log_handler = g_log_set_handler (NULL, G_LOG_LEVEL_CRITICAL | G_LOG_LEVEL_WARNING | G_LOG_FLAG_FATAL
