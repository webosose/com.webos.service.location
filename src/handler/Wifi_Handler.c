 ...
 * @return    int
 */
void wifi_handler_tracking_cb(gboolean enable_cb, Position *position, Accuracy *accuracy, int error, gpointer privateIns, int type)
{
 ...
 * @return    int
 */
void wifi_handler_position_cb(gboolean enable_cb, Position *position, Accuracy *accuracy, int error, gpointer privateIns, int type)
{
...
static void wifi_handler_start_tracking(Handler *self, gboolean enable, StartTrackingCallBack track_cb, gpointer handlerobj)
{
    ...
    int result = ERROR_NONE;
    WifiHandlerPrivate* priv = WIFI_HANDLER_GET_PRIVATE(self);
    if (priv == NULL || (priv->wifi_plugin->ops.start_tracking) == NULL) track_cb(TRUE, NULL, NULL, ERROR_NOT_AVAILABLE, NULL, HANDLER_WIFI);
...
static void wifi_handler_init(WifiHandler *self)
{
    ...
    priv->log_handler = g_log_set_handler(NULL, G_LOG_LEVEL_CRITICAL | G_LOG_FLAG_FATAL | G_LOG_FLAG_RECURSION, lsloghandler, NULL);
}
