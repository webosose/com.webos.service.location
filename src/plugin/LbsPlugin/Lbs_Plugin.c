...
int get_geocode(gpointer handle, const Address* address, Position *pos, Accuracy *ac)
{
    ...
    if (address->street != NULL) geoclue_address_details_insert(geoclue_addrress, GEOCLUE_ADDRESS_KEY_STREET, address->street);
    if (address->locality != NULL) geoclue_address_details_insert(geoclue_addrress, GEOCLUE_ADDRESS_KEY_LOCALITY, address->locality);
    if (address->region != NULL) geoclue_address_details_insert(geoclue_addrress, GEOCLUE_ADDRESS_KEY_REGION, address->region);
    if (address->countrycode != NULL) geoclue_address_details_insert(geoclue_addrress, GEOCLUE_ADDRESS_KEY_COUNTRY, address->countrycode);
    if (address->postcode != NULL) geoclue_address_details_insert(geoclue_addrress, GEOCLUE_ADDRESS_KEY_POSTALCODE, address->postcode);
    LS_LOG_DEBUG(
