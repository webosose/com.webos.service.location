...
static void position_cb(GeocluePositionGps *position, GeocluePositionGpsFields fields, int timestamp, double latitude, double longitude,
                        double altitude, double speed, double direction, double climb, GeoclueAccuracy *accuracy, gpointer userdata);

static void satellite_cb(GeoclueSatellite *satellite, guint64 used_in_fix, int visible_satellites, guint64 ephemeris_info, guint64 almanac_info,
 ...
 * @return    void
 */
static void position_cb_async(GeocluePositionGps *position, GeocluePositionGpsFields fields, int timestamp, double latitude, double longitude,
                              ...
                              gpointer userdata)
{
    ...
    if (error) {
        ...
        g_signal_handlers_disconnect_by_func(G_OBJECT(GEOCLUE_PROVIDER(plugin_data->geoclue_pos)), G_CALLBACK(position_cb), plugin_data);
 ...
 * @return    void
 */
static void tracking_cb(GeocluePositionGps *position, GeocluePositionGpsFields fields, int timestamp, double latitude, double longitude,
...
static int get_position(gpointer handle, PositionCallback positionCB)
{
    ...
    if (send_geoclue_command(geoclueGps->geoclue_pos, "REQUESTED_STATE", "SINGLEUPDATES") == FALSE) return ERROR_NOT_AVAILABLE;
...
static int get_gps_data(gpointer handle, gboolean enable_data, gpointer gps_cb, int datatype)
{
    ...
    case HANDLER_DATA_NMEA:
    {
        ...
        } else g_signal_handlers_disconnect_by_func(G_OBJECT(GEOCLUE_PROVIDER(geoclueGps->geoclue_nmea)), G_CALLBACK(nmea_callback), geoclueGps);
    }
        break;
    case HANDLER_DATA_SATELLITE:
    {
        ...
        } else g_signal_handlers_disconnect_by_func(G_OBJECT(GEOCLUE_PROVIDER(geoclueGps->geoclue_satellite)), G_CALLBACK(satellite_cb), geoclueGps);
    }
        break;
    case HANDLER_DATA_POSITION:
    {
        ...
        if (enable_data) {
            ...
            if (send_geoclue_command(geoclueGps->geoclue_pos, "REQUESTED_STATE", "PERIODICUPDATES") == FALSE) return ERROR_NOT_AVAILABLE;
...
gpsd->log_handler = g_log_set_handler (NULL, G_LOG_LEVEL_CRITICAL | G_LOG_LEVEL_WARNING | G_LOG_FLAG_FATAL
