...
void cell_handler_tracking_cb(gboolean enable_cb, Position *position, Accuracy *accuracy, int error, gpointer privateIns, int type)
{
    ...
    if (position != NULL) {
        ...
        cell_set_stored_position(position->timestamp, position->longitude, position->latitude, accuracy->horizAccuracy, accuracy->vertAccuracy);
    }
...
static void cell_data_cb(LSHandle *sh, LSMessage *reply, void *ctx)
{
    ...
    struct json_object *act_obj = NULL;
    ...
    struct json_object *event_obj = NULL;
    unsigned char act;
    int mcc;
    int mnc;
    char *full_name;
    ...
    g_return_if_fail(priv);
    if (priv->is_started == FALSE) return;
 ...
 * @return    int
 */
static void cell_handler_start_tracking(Handler *self, gboolean enable, StartTrackingCallBack track_cb, gpointer handlerobj, int handlertype,
