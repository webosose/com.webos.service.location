...
LSMethod LocationService::rootMethod[] = { { "getNmeaData", LocationService::_getNmeaData }, { "getCurrentPosition",
                                           ...
                                           { "startTracking", LocationService::_startTracking }, { "getReverseLocation",
                                                                                                   LocationService::_getReverseLocation },
                                           { "getGeoCodeLocation", LocationService::_getGeoCodeLocation },
                                           { "getAllLocationHandlers", LocationService::_getAllLocationHandlers }, { "SetGpsStatus",
...
bool LocationService::getNmeaData(LSHandle *sh, LSMessage *message, void *data)
{
    ...
    if (handler_array[HANDLER_GPS] != NULL) ret = handler_start(HANDLER_INTERFACE(handler_array[HANDLER_GPS]), HANDLER_TYPE_GPS);
    else {
...
bool LocationService::getCurrentPosition(LSHandle *sh, LSMessage *message, void *data)
{
    ...
    if (m_JsonArgument == NULL || is_error(m_JsonArgument)) {
        ...
        LSMessageReplyError(sh, message, TRACKING_UNKNOWN_ERROR);
        if (m_JsonArgument != NULL) json_object_put(m_JsonArgument);
        return true;
    }
    ...
    if ((isGpsReqSucess == false) && (isWifiReqSucess == false) && (isCellIdReqSucess == false)) {
        ...
        LS_LOG_DEBUG(
                "Handler Started are gpsHandlerStarted=%d,wifiHandlerStarted=%d,cellidHandlerStarted=%d", isGpsReqSucess, isWifiReqSucess, isCellIdReqSucess);
        if (serviceObject != NULL) json_object_put(serviceObject);
        return true;
    }
}
bool LocationService::reqLocationToHandler(int handler_type, unsigned char *reqHandlerType, int subHandlerType, LSHandle *sh)
{
    int ret;
    ...
    if (ret) {
        ...
        ret = ::getCurrentPosition(HANDLER_INTERFACE(handler_array[handler_type]), START, wrapper_getCurrentPosition_cb, subHandlerType, sh);
        if (ret == ERROR_NOT_AVAILABLE) {
            LS_LOG_DEBUG("getCurrentPosition failed errorcode %d", ret);
            ::getCurrentPosition(HANDLER_INTERFACE(handler_array[handler_type]), STOP, wrapper_getCurrentPosition_cb, subHandlerType, sh);
 ...
 * @return    bool, if successful return true else false
 */
bool LocationService::readLocationfromCache(LSHandle *sh, LSMessage *message, json_object *serviceObject, int maxAge, unsigned char handlerstatus)
{
    ...
    if (getCachedDatafromHandler(HANDLER_INTERFACE(handler_array[HANDLER_GPS]), &gpspos, &gpsacc, HANDLER_GPS)) gpsCacheSuccess = TRUE;
    /*if(getCachedDatafromHandler(HANDLER_INTERFACE(handler_array[HANDLER_NW]),&wifipos,&wifiacc,HANDLER_WIFI))
     wifiCacheSuccess = TRUE;*/

    if (getCachedDatafromHandler(HANDLER_INTERFACE(handler_array[HANDLER_NW]), &cellidpos, &cellidacc, HANDLER_CELLID)) cellCacheSucess = TRUE;
 ...
 */
void LocationService::handler_Selection(LSHandle *sh, LSMessage *message, void *data, int *accuracy, int *responseTime, unsigned char *handlerStatus)
{
...
bool LocationService::getReverseLocation(LSHandle *sh, LSMessage *message, void *data)
{
    ...
    if (ret == ERROR_NONE) {
        ...
        ::getReverseLocation(HANDLER_INTERFACE(handler_array[HANDLER_LBS]), &pos, &address);
        ...
        if (address.countrycode != NULL) json_object_object_add(serviceObject, "countrycode", json_object_new_string(address.countrycode));
        else json_object_object_add(serviceObject, "countrycode", json_object_new_string(""));

        if (address.postcode != NULL) json_object_object_add(serviceObject, "postcode", json_object_new_string(address.postcode));
        ...
        if (address.locality != NULL) json_object_object_add(serviceObject, "locality", json_object_new_string(address.locality));
...
bool LocationService::getGeoCodeLocation(LSHandle *sh, LSMessage *message, void *data)
{
    ...
    m_JsonArgument = json_tokener_parse((const char*) LSMessageGetPayload(message));
    if (m_JsonArgument == NULL || is_error(m_JsonArgument)) {
        LSMessageReplyError(sh, message, GEO_CODE_LOCATION_INVALID_INPUT);
        if (m_JsonArgument != NULL) json_object_put(m_JsonArgument);
        return true;
    }
    ...
    ret = handler_start(HANDLER_INTERFACE(handler_array[HANDLER_LBS]), HANDLER_LBS);
    if (ret == ERROR_NONE) {
        ::getGeoCodeLocation(HANDLER_INTERFACE(handler_array[HANDLER_LBS]), &addr, &pos, &acc);
...
bool LocationService::getAllLocationHandlers(LSHandle *sh, LSMessage *message, void *data)
{
    ...
    if (m_JsonArgument == NULL || is_error(m_JsonArgument)) {
        json_object_put(m_JsonArgument);
        LSMessageReplyError(sh, message, HANDLER_START_FAILURE);
        if (m_JsonArgument != NULL) json_object_put(m_JsonArgument);
        return true;
    }
...
bool LocationService::getLocationHandlerDetails(LSHandle *sh, LSMessage *message, void *data)
{
    ...
    if (NULL == handler) {
        ...
        LSMessageReplyError(sh, message, GET_HANDLER_DETAILS_INVALID_PARAMETER);
    }
    if (strcmp(handler, "gps") == 0) {
        ...
        json_object_object_add(serviceObject, "errorCode", json_object_new_int(GET_HANDLER_DETAILS_SUCCESS));
        mRetVal = LSMessageReply(sh, message, json_object_to_json_string(serviceObject), &mLSError);
        if (mRetVal == false) {
            LSErrorPrintAndFree(&mLSError);
        }
        ...
        LS_LOG_DEBUG("LPAppGetHandle is not created");
        LSMessageReplyError(sh, message, GET_HANDLER_DETAILS_NOT_SUPPORTED);
        return true;
    }
...
bool LocationService::getGpsSatelliteData(LSHandle *sh, LSMessage *message, void *data)
{
    ...
    ret = ::getGpsSatelliteData(handler_array[HANDLER_GPS], START, wrapper_getGpsSatelliteData_cb);
    if (ret != ERROR_NONE) {
        LSMessageReplyError(sh, message, GET_SAT_DATA_UNKNOWN_ERROR);
    }
    return true;
...
{
    long long mTTFF = ::getTimeToFirstFix((Handler*) handler_array[HANDLER_GPS]);
    LSError mLSError;
    char str[MAX_LIMIT];
...
{
    LS_LOG_DEBUG(
            "value of locality  %s region %s country %s country coede %s area %s street %s postcode %s", address->locality, address->region, address->country, address->countrycode, address->area, address->street, address->postcode);
}
void getGeoCodeLocation_cb(Position* position, Accuracy *accuracy, int error, gpointer data)
{
    LS_LOG_DEBUG( "value of lattitude %f longitude %f atitude %f", position->latitude, position->longitude, position->altitude);
}
...
void LocationService::stopSubcription(const char* key)
{
    if (strcmp(key, SUBSC_START_TRACK_KEY) == 0) {
        if (getHandlerStatus(GPS)) {
            ::startTracking(handler_array[HANDLER_GPS], STOP, wrapper_startTracking_cb, HANDLER_GPS, NULL);
            handler_stop(handler_array[HANDLER_GPS], HANDLER_GPS);
        }
        if (getHandlerStatus(NETWORK)) {
            ::startTracking(handler_array[HANDLER_NW], STOP, wrapper_startTracking_cb, HANDLER_CELLID, NULL);
            handler_stop(handler_array[HANDLER_NW], HANDLER_CELLID);
            ::startTracking(handler_array[HANDLER_NW], STOP, wrapper_startTracking_cb, HANDLER_WIFI, NULL);
