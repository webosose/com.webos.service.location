 ...
 */
    static void
unref_plugin (Plugin_Prop* plugin)
{
 ...
 */
    static gboolean
load_plugin_symbols (Plugin_Prop* plugin_prop,
...
void plugin_free(gpointer plugin, const char* plugin_name) {
    ...
    gboolean
plugin_is_supported(const char *plugin_name)
{
    if(!plugin_name)
        return FALSE;
    ...
    for(index = -1 ; index < PLUGIN_MAX_INDEX ; index++){
        if(index >= 0){
            g_snprintf(name, 20, "%s%d", plugin_name, index);
        }else{
            g_snprintf(name, 20, "%s", plugin_name);
        }

        ret = is_supported_plugin(name);
        if(ret == TRUE) {
            found = TRUE;
            LS_LOG_DEBUG("module name(%s) is found", name);
            break;
        }
    }
...
gboolean is_supported_plugin(const char *plugin_name) {
    ...
    unref_plugin(plugin);
    LS_LOG_DEBUG("plugin name(%s) is found", name);
    return TRUE;
}
