...
static void satellite_cb(GeoclueSatellite           *satellite,
                         ...
                         guint64                    ephemeris_info,
                         guint64                    almanac_info,
                         GPtrArray                  *sat_data,
                         gpointer                   userdata);
                              ...
                              gpointer                  userdata)
{
    ...
    if (error) {
        ...
        g_signal_handlers_disconnect_by_func(G_OBJECT(GEOCLUE_PROVIDER(plugin_data->geoclue_pos)),
                G_CALLBACK(position_cb), plugin_data);
        ...
        gpointer              userdata)
{
    ...
    for (index = 0; index < visible_satellites; index++) {
        ...
        (( used_in_fix & (1 << prn - 1))) == 0 ? (used = FALSE) : ( used= TRUE);
        (( ephemeris_info & (1 << prn - 1))) == 0 ? (hasephemeris = FALSE) : (hasephemeris = TRUE);
        (( almanac_info & (1 << prn - 1))) == 0 ? (hasalmanac = FALSE) : (hasalmanac = TRUE);
 ...
 * @return    void
 */
static void tracking_cb (GeocluePositionGps      *position,GeocluePositionGpsFields fields,
        int                   timestamp,
        double                latitude,
        double                longitude,
        double                altitude,
...
static int get_position(gpointer handle, PositionCallback positionCB){
    ...
    if(send_geoclue_command(geoclueGps->geoclue_pos ,"REQUESTED_STATE","SINGLEUPDATES") == FALSE)
